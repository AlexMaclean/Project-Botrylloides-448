...
plot.length.n50 <- function(name, data, x, xlab) {
  cols <- c("black", "blue", "purple")

  plot(x, data$total.length, col = cols[1], type = "l",
       ylim = c(0, max(data$total.length)), xlab = xlab, ylab = "Length (bp)")
  lines(x, data$N50, col = cols[2])
  lines(x, data$largest.contig, col = cols[3], lty = 2)

  title(main = paste(name, "Genome Quality Vs", xlab))
  legend("bottomleft",
         legend = c("Total Genome Length", "N50", "Largest Contig"),
         col = cols,
         pch = 19,
         inset = c(0.1, 0.1))
}
...
plot.num.contigs <- function(name, data, x, xlab) {
  plot(x, data$X..contigs, type = "p", pch = 5,
       ylim = c(0, max(data$X..contigs)), ylab = "# Contigs", xlab = xlab)
  title(main = paste(name, "Number of Contigs Vs", xlab))
}

# Plots all the run times against the provided x-axis, also take a position for the legend
plot.runtimes <- function(name, data, x, xlab, lpos, linset) {
  ty <- "o"
  ...
  title(main = paste(name, "Runtimes Vs", xlab))
